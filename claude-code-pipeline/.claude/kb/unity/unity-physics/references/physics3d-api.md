...
public class PhysicsMovement : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (Input.GetKey(KeyCode.W))
            _rb.AddRelativeForce(Vector3.forward * thrustForce);

        // Apply torque for steering
        float turn = Input.GetAxis("Horizontal");
        _rb.AddTorque(Vector3.up * turn * torqueForce);
    }
}
```
...
if (Physics.Raycast(transform.position, -Vector3.up, out hit, 100.0f))
    GameDebug.Log("Distance: " + hit.distance);
...
if (Physics.Raycast(transform.position, transform.forward, out hit, 50f, enemyMask))
    GameDebug.Log("Enemy hit: " + hit.collider.name);
```
...
for (int i = 0; i < hitCount; i++)
    GameDebug.Log(buffer[i].collider.name);
```
...
private void OnCollisionEnter(Collision other)
{
    ...
    GameDebug.Log("Relative velocity: " + other.relativeVelocity);
    GameDebug.Log("Hit object: " + other.gameObject.name);
}
...
{
    GameDebug.Log(other.name + " left trigger zone");
}
```
...
public class DoorObject : MonoBehaviour
{
    private void OnCollisionEnter(Collision other)
    {
        if (other.gameObject.CompareTag("Player"))
            GameDebug.Log("The player character has touched the door.");

        if (other.gameObject.CompareTag("Enemy"))
            GameDebug.Log("An enemy character has touched the door!");
    }
}
```
...
{
    [SerializeField] private Rigidbody connectedObject;
    [SerializeField] private float breakThreshold = 500f;
    ...
    {
        GameDebug.Log("Joint broke with force: " + breakForce);
    }
}
```
