...
public class SaveManager : MonoBehaviour
{
    ...
    public void Save(int slot = 0)
    {
        ...
        GameDebug.Log($"Game saved to slot {slot}");
    }
    ...
    public bool Load(int slot = 0)
    {
        ...
        if (!File.Exists(path))
        {
            GameDebug.LogWarning($"No save found in slot {slot}");
            return false;
        }
        ...
        try
        {
            ...
            ApplyData(data);
            GameDebug.Log($"Game loaded from slot {slot}");
            return true;
        }
        catch (Exception e)
        {
            GameDebug.LogError($"Failed to load slot {slot}: {e.Message}");

            // Try backup
            string backupPath = path + ".bak";
            if (File.Exists(backupPath))
            {
                GameDebug.Log("Attempting to load from backup...");
                try
                {
                ...
                {
                    GameDebug.LogError($"Backup also failed: {backupEx.Message}");
                }
            }
    ...
    private GameSaveData GatherData()
    {
...
public async Awaitable SaveAsync(int slot, CancellationToken token = default)
{
    ...
    GameDebug.Log($"Async save to slot {slot} complete");
}
```
...
public static class GameSettings
{
    ...
    private static void SetInt(string key, int val)
    {
