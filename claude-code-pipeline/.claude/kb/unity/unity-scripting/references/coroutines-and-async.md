...
IEnumerator SpawnWaves()
{
    ...
    GameDebug.Log("All waves complete");
}
```
...
public class AsyncExample : MonoBehaviour
{
    ...
    private async Awaitable DoWorkAsync(CancellationToken token)
    {
        while (true)
        {
...
New code follows the project's async standard (UniTask or `Awaitable`, `project_setup.md` §1); coroutines are not the default.
