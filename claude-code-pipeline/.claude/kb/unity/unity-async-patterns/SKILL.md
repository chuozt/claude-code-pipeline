...
async Awaitable LoadAndProcess()
{
    ...
    {
        GameDebug.LogError($"Load failed: {e}");
    }
}
...
IEnumerator LoadAndProcessCoroutine()
{
    ...
    if (!isLoaded)
    {
        GameDebug.LogError("Load failed");
        yield break;
    }
...
IEnumerator CaptureScreenshot()
{
    ...
    if (Application.isBatchMode)
    {
        ...
        GameDebug.LogWarning("Screenshot not available in batch mode");
        yield break;
    }
...
async Awaitable Start()
{
    ...
    {
        GameDebug.LogError($"Work failed: {e.Message}");
    }
}
