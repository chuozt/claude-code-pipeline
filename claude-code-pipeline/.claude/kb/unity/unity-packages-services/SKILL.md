...
public class ServicesInitializer : MonoBehaviour
{
    async void Start()
    {
        ...
        GameDebug.Log("Unity Services initialized");
    }
}
```
...
public class AuthManager : MonoBehaviour
{
    async void Start()
    {
        ...
        {
            GameDebug.Log($"Signed in. Player ID: {AuthenticationService.Instance.PlayerId}");
        };
...
public class CloudSaveManager : MonoBehaviour
{
    async void Start()
    {
        ...
        GameDebug.Log("Data saved to cloud");
        ...
        if (loadedData.TryGetValue("playerLevel", out var levelItem))
        {
            ...
            GameDebug.Log($"Player level: {level}");
        }
    }
}
```
...
public class GameBootstrap : MonoBehaviour
{
    async void Awake()
    {
        try
        {
            await UnityServices.InitializeAsync();
            await AuthenticationService.Instance.SignInAnonymouslyAsync();
            GameDebug.Log($"Player ID: {AuthenticationService.Instance.PlayerId}");

            // Now safe to use Cloud Save, Analytics, etc.
        }
        catch (ServicesInitializationException e)
        {
            GameDebug.LogError($"Services failed to initialize: {e.Message}");
        }
        catch (AuthenticationException e)
        {
            GameDebug.LogError($"Authentication failed: {e.Message}");
        }
    }
}
```
