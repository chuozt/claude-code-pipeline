...
{
    isGrounded = true; // May MISS if sphere starts inside the ground collider
}
```
...
isGrounded = Physics.CheckSphere(feetPosition, radius, groundMask);
...
private void DetectHits()
{
    ...
    if (count == _hitBuffer.Length)
        GameDebug.LogWarning("Hit buffer full -- may have missed results");
}
```
