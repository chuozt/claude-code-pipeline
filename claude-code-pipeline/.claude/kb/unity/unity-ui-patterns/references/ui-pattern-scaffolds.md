...
public class ScreenManager : MonoBehaviour
{
    ...
    public static ScreenManager Instance => s_instance;
    ...
    public async Awaitable PopAsync(
        ...
        IScreenTransition showTransition = null)
    {
        if (_isTransitioning || _screenStack.Count <= 1)
            return;
...
public class HudController : MonoBehaviour
{
    ...
    {
        if (!_registeredNames.Add(widget.WidgetName))
            return;
...
public class UIFeedback : ScriptableObject
{
    ...
    [SerializeField] private string ussClassName = "feedback--damage";
    ...
    public string UssClassName => ussClassName;
    public string TargetElementName => targetElementName;
...
public class UIFeedbackPlayer : MonoBehaviour
{
    ...
    public async Awaitable PlayAsync(UIFeedback feedback)
    {
        ...
        if (target == null)
        {
            GameDebug.LogWarning($"UIFeedbackPlayer: target '{feedback.TargetElementName}' not found.");
            return;
        }

        // Guard against duplicate non-stacking effects
        if (!feedback.CanStack
            && !_activeEffects.Add(feedback.UssClassName))
            return;
...
public class InventoryListSetup : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        ListViewHelper.Setup<ItemData>(
            ...
            onSelectionChanged: item =>
            {
                GameDebug.Log($"Selected: {item.Name}");
            },
            itemHeight: 50f
        );
    }
