...
UI Toolkit is Unity's retained-mode UI framework; use the UI system the project names in `project_setup.md` §1. It uses a retained-mode rendering model inspired by web technologies: UXML for structure, USS for styling, and C# for behavior.
...
public class UISetup : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        var button = new Button(() => GameDebug.Log("Clicked!"));
...
if (isEmpty)
    slotElement.AddToClassList("inventory__slot--empty");
```
...
{
    GameDebug.Log($"Pointer down at {evt.position}");
});

// Trickle-down -- handle before children
parent.RegisterCallback<PointerDownEvent>(evt =>
{
    GameDebug.Log("Parent intercepting before children");
...
slider.RegisterValueChangedCallback(evt =>
{
    ...
    GameDebug.Log($"Slider changed from {oldVal} to {newVal}");
});
...
private void HandleClick(ClickEvent evt, int data)
{
    GameDebug.Log($"Clicked with data: {data}");
}
```
...
public class ResizeManipulator : PointerManipulator
{
    ...
    private void OnPointerMove(PointerMoveEvent evt)
    {
        ...
        var delta = evt.position - _startPointer;
