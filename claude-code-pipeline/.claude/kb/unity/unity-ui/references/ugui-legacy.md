...
uGUI is Unity's GameObject-based UI system. Use the UI system the project names in `project_setup.md` §1; uGUI is fully supported in Unity 6 and is the runtime UI of many shipped projects. It uses Canvas, RectTransform, EventSystem, and a set of visual/interaction components.
    ...
    IDropHandler
{
    ...
    {
        GameDebug.Log($"Clicked: {gameObject.name}");
    }
    ...
    {
        GameDebug.Log("Drag ended");
    }

    public void OnDrop(PointerEventData eventData)
    {
        var dragged = eventData.pointerDrag;
        GameDebug.Log($"{dragged.name} dropped on {gameObject.name}");
    }
}
```
...
{
    GameDebug.Log($"Selected resolution index: {index}");
}
```
...
private void OnNameSubmitted(string text) => GameDebug.Log($"Submitted: {text}");
private void OnNameTyping(string text) => GameDebug.Log($"Typing: {text}");
```
...
private void OnScrollChanged(Vector2 normalizedPos)
{
    ...
    GameDebug.Log($"Scroll position: {normalizedPos}");
}
