...
public class OnDemandProbe : MonoBehaviour
{
    ...
    {
        if (_renderId >= 0
            && _probe.IsFinishedRendering(_renderId))
        {
            GameDebug.Log("Reflection probe render complete");
            _renderId = -1;
        }
    }
}
...
public class DynamicLightController : MonoBehaviour
{
    ...
    void Start()
    {
        _sceneLight = GetComponent<Light>();
        _baseIntensity = _sceneLight.intensity;
    }

    void Update()
    {
        // Perlin noise gives a smooth flicker instead of random jumps
        float noise = Mathf.PerlinNoise(Time.time * flickerSpeed, 0f);
        _sceneLight.intensity = _baseIntensity + (noise - 0.5f) * flickerAmount;
    }

    public void SetColor(Color newColor)
    {
        _sceneLight.color = newColor;
    }

    public void SetShadowsEnabled(bool isEnabled)
    {
        _sceneLight.shadows = isEnabled ? LightShadows.Soft : LightShadows.None;
    }
}
```
...
public class DayNightCycle : MonoBehaviour
{
    ...
    private float _timeOfDay = 0.25f; // Start at sunrise

    void Update()
    {
        _timeOfDay += Time.deltaTime / dayDurationSeconds;
        if (_timeOfDay > 1f)
            _timeOfDay -= 1f;
        ...
        directionalLight.color = lightColorGradient.Evaluate(_timeOfDay);
        directionalLight.intensity = intensityCurve.Evaluate(_timeOfDay);
    }
}
```
