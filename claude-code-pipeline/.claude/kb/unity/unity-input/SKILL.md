...
void Update()
{
    foreach (var touch in Touch.activeTouches)
        GameDebug.Log($"{touch.touchId}: {touch.screenPosition}, {touch.phase}");
}
```
...
InputSystem.onDeviceChange += (device, change) =>
{
    if (change == InputDeviceChange.Added)
        GameDebug.Log($"Device connected: {device.displayName}");
};
```
...
public class GameStateManager : MonoBehaviour
{
    ...
    void Start() => _playerInput = GetComponent<PlayerInput>();

    public void EnterMenu() => _playerInput.SwitchCurrentActionMap("UI");
    public void ExitMenu() => _playerInput.SwitchCurrentActionMap("Player");
}
```
...
var rebindOp = action.PerformInteractiveRebinding()
    ...
    .OnComplete(op =>
    {
        GameDebug.Log($"Rebound to: {op.action.bindings[0].effectivePath}");
        op.Dispose();
    })
    .Start();
```
