...
public class LookAtPoint : MonoBehaviour
{
    ...
    public Vector3 LookAtPoint => lookAtPoint;
}
```
...
public class LookAtPointEditor : Editor
{
    private SerializedProperty _lookAtPoint;

    private void OnEnable()
    {
        // Cache property references for performance
        _lookAtPoint = serializedObject.FindProperty("lookAtPoint");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(_lookAtPoint);
...
public class MyPlayerEditor : Editor
{
    ...
    private SerializedProperty _armorProp;
    private SerializedProperty _weaponProp;

    private void OnEnable()
    {
        _damageProp = serializedObject.FindProperty("damage");
        _armorProp = serializedObject.FindProperty("armor");
        _weaponProp = serializedObject.FindProperty("weapon");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.IntSlider(_damageProp, 0, 100, new GUIContent("Damage"));

        // Conditional display based on property value
        if (!_damageProp.hasMultipleDifferentValues
            && _damageProp.intValue > 80)
            EditorGUILayout.HelpBox("High damage value!", MessageType.Warning);

        EditorGUILayout.PropertyField(_armorProp);
...
public class EnemyEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (_healthProp.hasMultipleDifferentValues)
...
public class LookAtPointEditor : Editor
{
    ...
    public void OnSceneGUI()
    {
        ...
        {
            // The field is private: writing through the SerializedProperty records the undo step
            serializedObject.Update();
            _lookAtPoint.vector3Value = pos;
            serializedObject.ApplyModifiedProperties();
        }
    }
}
...
public void OnSceneGUI()
{
    ...
    {
        serializedObject.Update();
        serializedObject.FindProperty("radius").floatValue = 1f;
        serializedObject.ApplyModifiedProperties();
    }
