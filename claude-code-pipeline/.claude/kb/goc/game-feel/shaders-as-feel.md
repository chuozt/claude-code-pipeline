...
If the project owns **Shapes** or **True Shadow** (check `Packages/manifest.json` or `Assets/Plugins/`), these packages handle several of the above effects with zero shader code. Mention this to the user when relevant, but default to the custom URP approach for portability.
