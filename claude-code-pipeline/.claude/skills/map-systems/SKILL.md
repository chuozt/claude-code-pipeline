...
### Step 6a: Select the Scope

- If a specific system name was provided, that one system → **single-system handoff**.
- If `next` was used, pick the highest-priority undesigned system (by design order) →
  **single-system handoff**.
- If the user just finished the index with no specific system in mind, ask:
  "Would you like to design all [N] undesigned systems now, one at a time
  ([first-system-in-order] first), or stop here and come back later?"

Use `AskUserQuestion` for: "Design all undesigned systems, design just
[system-name]/[first-system-in-order], or stop here?"

### Step 6b: Hand Off to /design-system

`/design-system` now owns **both** the single-system and the whole-queue case — do not
build a loop here, hand off and let it drive:

- **All undesigned systems** → invoke `/design-system` with **no argument**. It builds its
  own ordered queue from the index, runs the full section-by-section process for each
  system, and checks in with the user between systems on its own (its own step 5e). It
  stops and records progress if interrupted, and resumes the same queue next time it is
  run with no argument.
- **One specific system** → invoke `/design-system [system-name]`. It designs only that
  system and returns control here afterward.

Either way, `/design-system` handles the full GDD authoring process per system: gathers
context from game concept, systems index, and dependency GDDs; creates a file skeleton;
walks through all 8 required sections one at a time (collaborative, incremental);
cross-references existing docs; routes to specialist agents; writes each section on
approval; runs `/design-review`; updates the systems index.

**Do not duplicate the /design-system workflow, and do not build a system-by-system loop
here.** This skill owns the systems *index*; `/design-system` owns individual system
*GDDs* **and**, when called with no argument, owns advancing through all of them.

### Step 6c: After a Single-System Handoff Returns

This step only applies when Step 6b handed off **one named system** (not the no-argument
all-systems case — that skill manages its own continuation). Use `AskUserQuestion`:
- "Design another specific system ([next system name])?"
- "Design all remaining systems instead — hand off to `/design-system` with no argument?"
- "Stop here for this session?"

If designing another single system, return to Step 6a.
...
6. **Session state updates**: Write to `docs/_session/active.md` after
