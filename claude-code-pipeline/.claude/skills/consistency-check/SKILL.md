...
description: "Fast numeric cross-check across ALL design docs: the same entity, item or formula carrying different values in different documents, found by grepping the entity registry first rather than reading every document. For one document in depth use /design-review; for a full design-quality review of the set use /review-all-gdds. Use when typing /consistency-check or saying 'do the numbers agree across docs', 'find conflicting values', 'did this change break other docs'."
