...
> "The stack is not filled in yet. Fill `CLAUDE.md` §1 and `project_setup.md` §1 first
> (`/project-overview` drafts them). Architecture cannot be written without the exact Unity
> version and stack."
...
```
⚠️  [ClassName.method()] — Unity <version> (post-cutoff, HIGH risk)
    Verified against: the matching `.claude/kb/` document, or an `mcp__unity-api__*` lookup
...
`MonoBehaviour`, `ScriptableObject`, `Awaitable` in Unity), flag the version and verify the type
...
2. **Gate check**: "The master architecture document is complete. Once all required ADRs
   are written, run `/architecture-review` to confirm nothing is left uncovered.""
3. **Update session state**: Write a summary to `docs/_session/active.md`
...
- Run `/architecture-review` when all required ADRs are written and the architecture is signed off
- **Puzzle / resource-flow game?** The designer's difficulty pipeline starts here:
  `/gd-map-flow`. See `/gd-workflow-help` for the five phases.
