---
name: art-bible
description: "Author the art bible section by section — the visual identity spec every asset follows (style, palette rules, shapes, UI look, asset standards for the target device). Run after the concept is settled and before asset production. Use when typing /art-bible or saying 'define the visual style', 'write the art bible', 'art direction doc'."
...
- Does `.claude/docs/technical-preferences.md` have its Engine field filled (not `<...>`)? → the stack is set, skip that option
...
- If the stack in `project_setup.md` §1 is not filled yet, fill it (render pipeline and target device shape the asset standards)
