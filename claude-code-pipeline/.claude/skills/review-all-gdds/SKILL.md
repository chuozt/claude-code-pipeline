...
description: "Holistic review of the WHOLE design set at once: contradictions between systems, stale references, ownership conflicts, formula incompatibilities, and design problems that only show across systems (dominant strategies, cognitive overload, pillar drift). Slowest and deepest of the three reviews — run once the MVP GDDs are written, before architecture. Use when typing /review-all-gdds or saying 'review all the design docs together', 'is the design set coherent'."
...
append to `docs/_session/active.md`:
