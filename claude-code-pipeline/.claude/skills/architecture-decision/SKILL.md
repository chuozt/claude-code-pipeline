...
2. Identify which template sections are present by scanning headings:
   - `## Status` — **BLOCKING if missing**: nothing downstream can tell an accepted ADR from a proposed one
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Fill in `project_setup.md` §1 first, or tell me the Unity version and stack."
...
| **References Consulted** | [List the docs read, e.g. `.claude/kb/unity/unity-physics/SKILL.md`, an `mcp__unity-api__*` lookup] |
...
4.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via Task to validate the drafted ADR:
   ...
   - If the Engine field is still `<...>`, skip this step
