...
If the section is unconfigured (still `<...>`), ask once:
...
>
> (Record it in `.claude/docs/technical-preferences.md` so you won't be asked again.)
...
After writing the skeleton, update `docs/_session/active.md` with:
...
After writing each section, update `docs/_session/active.md`.
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions when the screen is handed over.
...
> "This spec should be reviewed by the ux-designer agent before it enters the
> implementation pipeline. The Pre-Production gate requires all key screen specs
> to have a review verdict."

Then use `AskUserQuestion`:
- "Review this spec now, or do something else first?"
  - Options:
    - "Review this spec now with the ux-designer agent"
...
a review pass over all completed specs before committing to production."
...
1. Read `docs/_session/active.md` — it records the current screen
...
- Have the `ux-designer` agent review this spec before it enters the implementation pipeline
- Run `/ux-design [next-screen]` to continue designing remaining screens or flows
- Hand the approved UX specs to the developer for implementation once all key screens are done
