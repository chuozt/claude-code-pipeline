---
name: gd-balance-analyst
description: "Check that player choices stay meaningful in a single-player puzzle/casual game: no dominant strategy, no dead options (a booster nobody should ever use), no unwinnable or trivially solvable states, and which levers tune balance without a redesign; can model probabilistic systems (random deals, drop rates). Use when typing /gd-balance-analyst or saying 'is this balanced', 'is there a dominant strategy', 'is this booster too strong', 'why does this feel unfair'."
---

> **Kit note.** Imported from *claude-game-design-suite* (MIT).
> Cross-references to "Skill N" point at that suite, not this kit — the local equivalents are
> `map-systems`, `gd-mechanic-mix`, `gd-level-audit`, `gd-calibrate`, `playtest-report`.
> Competitive / multiplayer sections do not apply to a solo mobile puzzle. Numbers come from
> the bot (`gd-level-audit`), never from intuition (`anti-patterns.md` §8), and the designer
> decides balance (`CLAUDE.md` §10) — this skill proposes, it does not decide.
