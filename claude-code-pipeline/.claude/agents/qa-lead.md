...
- Run or verify the smoke check step passes before any build goes to manual QA

### QA Workflow Integration

**Your skills to use:**
- `/test-designing-guide` — design test cases for a feature before it is built
- the smoke check step — run before every QA hand-off
- `/playtest-report` — structure a playtest session's findings
...
- Pre-QA gate: Run the smoke check step; block hand-off if it fails
...
- Review story acceptance criteria before implementation starts (the story readiness step)
...
3. **Smoke Check Ownership**: Run the smoke check step before every build goes to manual QA.
