...
The project has already chosen: **use the UI system named in `.claude/project_setup.md` §1**
(most mobile projects: UGUI plus the project's own screen framework). Never introduce a second UI
system for new screens, and never recommend migrating. The UI Toolkit and UGUI sections below
apply only when that table names that system. UI Toolkit remains a fine choice for Editor tools
and inspectors, which are not game UI.
