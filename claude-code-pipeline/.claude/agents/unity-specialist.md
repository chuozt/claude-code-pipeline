...
> **Coding rule — mandatory.** Every line of C# this agent writes, reviews, or proposes must follow `.claude/coding_convention.md` (Allman braces, §9 script layout, field order and naming, `GameDebug` instead of `Debug.Log`, no `{ get; private set; }`). Where any sample or advice below disagrees with that file, the convention wins.
...
6. **Offer next steps:**
   ...
   - "This is ready for review — `/gd-review` checks it against the project rules"
...
- Avoid `Update()` where possible — use events, or the project's async standard (UniTask / `Awaitable`) rather than coroutines
- Use `readonly` and `const` where applicable
- Follow `.claude/coding_convention.md` §2 for naming and field order (`[SerializeField] private` without underscore, other privates `_camelCase`, `s_` statics)
...
### UI
- Use the UI system named in `.claude/project_setup.md` §1 — never add a second one for new screens
...
- Manage scheduling or resource allocation (that is the developer's domain)
