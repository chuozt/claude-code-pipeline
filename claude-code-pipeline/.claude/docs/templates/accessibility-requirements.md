...
>
> **When to update**: After each the gate check step pass, after any accessibility
...
| PC (Screen Reader) | JAWS / NVDA / Windows Narrator | [Menu navigation announcements] | Not Started | Requires UI elements to expose accessible names and roles via platform UI layer. Unity has no built-in screen-reader bridge for game UI; a plugin or native accessibility layer is needed |
...
|----------|-------|----------|-----------|
| [Does the project's Unity version and UI system support dynamic accessibility node updates for HUD elements, or only static menus?] | [ux-designer] | [Before Technical Setup gate] | [Unresolved — check the Unity manual for the pinned version] |
