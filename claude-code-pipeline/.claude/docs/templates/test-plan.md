...
> **Date**: [date]
> **Written by**: the qa-lead agent or by hand
...
Critical paths to verify before QA hand-off (run via the smoke check step):
...
- [ ] Smoke check passes (start → win and start → lose on a Development build before QA hand-off)
- [ ] No regressions introduced — previous sprint's features still pass
- [ ] Code reviewed (via the code review step or documented peer review)
- [ ] Story file updated to `Status: Complete` via the story done step
...
*Generated by: the qa plan step — do not edit this line*
