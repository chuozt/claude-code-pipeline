...
{
    private static SceneCoordinator s_instance;

    private readonly HashSet<string> _loadedScenes = new();
    private string _activeContentScene;

    public static SceneCoordinator Instance => s_instance;
        ...
        CancellationToken token = default)
    {
        ...
        {
            GameDebug.LogWarning($"Scene '{sceneName}' is already loaded");
            return;
        }
    ...
    public async Awaitable UnloadScene(string sceneName, CancellationToken token = default)
    {
        if (!_loadedScenes.Contains(sceneName))
        {
            GameDebug.LogWarning($"Scene '{sceneName}' is not loaded");
            return;
        }
...
public class ProjectileLauncher : MonoBehaviour
{
    ...
    {
        if (_projHandle.IsValid())
            Addressables.Release(_projHandle);
...
var handle = Addressables.LoadAssetsAsync<GameObject>("enemy",
    asset => GameDebug.Log($"Loaded enemy: {asset.name}"));
