...
public class FindingObjects : MonoBehaviour
{
    private void Start()
    {
        ...
        if (gameObject.CompareTag("Player"))
            GameDebug.Log("This is the player");
    }
}
```
...
public class SceneLoader : MonoBehaviour
{
    ...
    {
        GameDebug.Log("Loaded: " + scene.name);
    }
    ...
    public void GetSceneInfo()
    {
        ...
        GameDebug.Log("Active scene: " + active.name);
        GameDebug.Log("Loaded scene count: " + SceneManager.loadedSceneCount);
    }
}
```
...
public class RespawnSystem : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Tag lookups walk the scene: resolve once, never per frame
        _respawn = GameObject.FindWithTag("Respawn");
    }

    private void Update()
    {
        if (_respawn != null)
            Instantiate(respawnPrefab, _respawn.transform.position, _respawn.transform.rotation);
    }
}
```
...
public class ToggleVisibility : MonoBehaviour
{
    ...
    public void Toggle()
    {
        ...
        GameDebug.Log("Self: " + target.activeSelf);
        GameDebug.Log("InHierarchy: " + target.activeInHierarchy);
    }
}
```
...
public class LayerRaycast : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Name-to-layer lookups are string work: resolve the masks once
        int groundLayer = LayerMask.NameToLayer("Ground");
        _groundMask = 1 << groundLayer;

        // Use GetMask for multiple layers
        _combinedMask = LayerMask.GetMask("Ground", "Water");
    }

    private void Update()
    {
        // Raycast only against the Ground layer
        if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, 100f, _groundMask))
            GameDebug.Log("Hit ground at: " + hit.point);

        Physics.Raycast(transform.position, Vector3.forward, 50f, _combinedMask);
    }
}
```
...
{
    [SerializeField] private EnemyConfig config; // All instances share the same asset
}
```
