...
if (Physics.Raycast(transform.position, fwd, 10))
    GameDebug.Log("Something ahead!");

// With hit info
RaycastHit hit;
if (Physics.Raycast(transform.position, -Vector3.up, out hit, 100.0f))
    GameDebug.Log("Distance to ground: " + hit.distance);

// With layer mask
int layerMask = 1 << LayerMask.NameToLayer("Enemies");
if (Physics.Raycast(origin, direction, out hit, maxDist, layerMask))
    GameDebug.Log("Hit enemy: " + hit.collider.gameObject.name);
...
private void OnCollisionEnter(Collision other)
{
    if (other.gameObject.CompareTag("Player"))
        GameDebug.Log("Player hit this object");
}

// Every physics frame while touching
...
```csharp
// Entered trigger volume
private void OnTriggerEnter(Collider other)
{
    if (other.CompareTag("Player"))
        GameDebug.Log("Player entered trigger zone");
}

// Each frame inside trigger
...
public bool IsGrounded()
{
    ...
    return Physics.SphereCast(transform.position, radius, Vector3.down,
        out _, (_capsule.height / 2f) - radius + 0.1f,
...
{
    [SerializeField] private float hoverForce = 12f;
...
private void DetectEnemies()
{
    int count = Physics.RaycastNonAlloc(transform.position, transform.forward,
        _hits, 50f, LayerMask.GetMask("Enemies"));

    for (int i = 0; i < count; i++)
        GameDebug.Log("Hit: " + _hits[i].collider.name);
}
```
...
private void Update()
{
    _rb.AddForce(Vector3.forward * 10f); // WRONG
}

// GOOD: Use FixedUpdate for physics
private void FixedUpdate()
{
    _rb.AddForce(Vector3.forward * 10f); // CORRECT
}
```
...
// GOOD: Cache in Awake(), reuse in callbacks
```
...
For non-physics-driven character movement (FPS controllers, NPCs), use `CharacterController` instead of Rigidbody. It handles slopes, steps, and collision sliding without physics simulation.
