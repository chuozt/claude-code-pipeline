...
void Update()
{
    ...
    foreach (var touch in Touch.activeTouches)
        GameDebug.Log($"ID:{touch.touchId} Pos:{touch.screenPosition} Phase:{touch.phase}");

    // By finger
    foreach (var finger in Touch.activeFingers)
    {
        var touch = finger.currentTouch;
        GameDebug.Log($"Finger {finger.index}: {touch.screenPosition}");
    }
}
```
...
{
    // Public on purpose: the Input System fills interaction parameters by reflection
...
var rebindOp = action.PerformInteractiveRebinding()
    ...
    .OnComplete(operation =>
    {
        ...
        GameDebug.Log($"Rebound to: {newPath}");
        operation.Dispose(); // Always dispose
    })
    .OnCancel(operation =>
    {
        GameDebug.Log("Rebinding cancelled");
        operation.Dispose();
    })
    .Start();
...
InputSystem.onDeviceChange += (device, change) =>
    GameDebug.Log($"Device {change}: {device.displayName} ({device.GetType().Name})");

// List all devices
foreach (var device in InputSystem.devices)
    GameDebug.Log($"{device.displayName}: {device.GetType().Name}");

// Check action state
GameDebug.Log($"Move phase: {moveAction.phase}, value: {moveAction.ReadValue<Vector2>()}");
```
