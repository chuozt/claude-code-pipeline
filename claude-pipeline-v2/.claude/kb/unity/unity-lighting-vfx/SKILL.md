...
public class ParticleController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Module structs write straight to native code -- cache in a local first
...
public class VFXController : MonoBehaviour
{
    ...
    private VFXEventAttribute _eventAttr;

    void Start()
    {
        _vfx = GetComponent<VisualEffect>();
        _eventAttr = _vfx.CreateVFXEventAttribute();

        _vfx.SetFloat("SpawnRate", 100f);
        _vfx.SetVector3("Direction", Vector3.up);
        _vfx.playRate = 1.5f;
    }

    void OnTriggerEnter(Collider other)
    {
        _eventAttr.SetVector3("position", other.transform.position);
        _vfx.SendEvent("OnHit", _eventAttr);
    }
...
public class ProbeRefresher : MonoBehaviour
{
    ...
    public bool IsReady()
    {
        return _probe.IsFinishedRendering(_probe.RenderProbe());
    }
}
```
