...
{
    [SerializeField] private ExplorationManager exploration;
    ...
    private void Start()
    {
        exploration.Enable();
        inventory.Enable();
        combat.Enable(); // all systems active from frame 1
    }
}
```
...
public class ConditionEvaluator : MonoBehaviour
{
    ...
    [SerializeField] private List<WinConditionBase> winConditions;
    [SerializeField] private CompositionMode winMode = CompositionMode.AllRequired;

    [SerializeField] private List<LoseConditionBase> loseConditions;
...
{
    public static int s_totalXP; // static, no reset safety, no persistence
    ...
    public void TakeDamage(int dmg)
    {
        ...
        if (hp <= 0)
        {
            s_totalXP += 50; // meta progression buried in combat code
            Destroy(gameObject);
        }
    }
}
...
{
    private int _maxEnemies = 10;         // magic number
    private float _spawnRate = 2f;        // magic number
    private float _enemyHealthMult = 1f;  // magic number
...
public class PacingDirector : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float normalized = Mathf.Clamp01(elapsed / config.SessionDurationSeconds);

        _currentIntensity = config.IntensityCurve.Evaluate(normalized);
        OnIntensityChanged?.Invoke(_currentIntensity);

        float[] thresholds = config.IntensityThresholds;

        for (int i = 0; i < thresholds.Length && i < _thresholdsFired.Length; i++)
        {
            if (_thresholdsFired[i]
                || _currentIntensity < thresholds[i])
                continue;

            _thresholdsFired[i] = true;
            OnThresholdCrossed?.Invoke(thresholds[i]);
        }
    }
