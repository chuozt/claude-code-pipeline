...
public class PerformanceOverlay : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        // Debug overlay only; production UI goes through the UI system named in project_setup.md §1
...
{
    private static readonly ProfilerCounter<int> s_enemyCount =
        new(ProfilerCategory.Scripts, "Active Enemies", ProfilerMarkerDataUnit.Count);
    private static readonly ProfilerCounter<int> s_projectileCount =
        new(ProfilerCategory.Scripts, "Projectiles", ProfilerMarkerDataUnit.Count);

    private void Update()
    {
        s_enemyCount.Sample(EnemyManager.ActiveCount);
        s_projectileCount.Sample(ProjectileManager.ActiveCount);
    }
}
```
