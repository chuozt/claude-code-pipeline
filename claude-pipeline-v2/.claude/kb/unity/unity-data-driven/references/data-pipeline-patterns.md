...
public class EnemyDataImporter : EditorWindow
{
    ...
    private string _outputFolder = "Assets/Data/Enemies";
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Import") && _csvFile != null)
            ImportCSV();
    }
    ...
    private void ImportCSV()
    {
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            // The config fields are [SerializeField] private, so write them through SerializedObject
            var serialized = new SerializedObject(config);
            serialized.FindProperty("maxHealth").intValue = int.Parse(values[1]);
            serialized.FindProperty("moveSpeed").floatValue = float.Parse(values[2]);
            serialized.FindProperty("attackDamage").floatValue = float.Parse(values[3]);
            serialized.FindProperty("detectionRange").floatValue = float.Parse(values[4]);
            serialized.FindProperty("xpReward").intValue = int.Parse(values[5]);
        ...
        GameDebug.Log($"Imported {created} enemy configs from CSV");
    }
}
#endif
```
