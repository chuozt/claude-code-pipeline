...
description: Unity UI patterns including Canvas optimization, list virtualization, and mobile-friendly UI.
---
...
public class CanvasOrganizer : MonoBehaviour
{
    ...
    [SerializeField] private Canvas staticCanvas;  // Background, frames

    [Header("Semi-Dynamic - Changes occasionally")]
    [SerializeField] private Canvas dynamicCanvas; // Buttons, icons

    [Header("Highly Dynamic - Changes every frame")]
    [SerializeField] private Canvas hudCanvas;     // Health, mana, timers

    private void Start()
    {
        // Nothing on the static canvas is clickable, so its raycaster is pure cost
        staticCanvas.GetComponent<GraphicRaycaster>().enabled = false;
    }
}
```
    ...
    where TView : Component
{
    ...
    [SerializeField] private TView itemPrefab;
    ...
    private void RefreshVisibleItems()
    {
        ...
        int newFirstVisible = Mathf.Max(0, Mathf.FloorToInt(scrollY / itemHeight));
        int visibleCount = Mathf.CeilToInt(_viewportHeight / itemHeight) + 2;
    ...
    private void PositionItem(TView item, int index)
    {
        ...
        rect.anchoredPosition = new Vector2(0, -index * itemHeight);
    ...
    {
        var rect = item.GetComponent<RectTransform>();
        return Mathf.RoundToInt(-rect.anchoredPosition.y / itemHeight);
    }
}
```
...
{
    [SerializeField] private VirtualizedList<ItemData, InventorySlot> list;

    public void ShowInventory(List<ItemData> items)
    {
        list.Initialize(items, BindSlot);
    }
...
public class SafeText : MonoBehaviour
{
    ...
    // Assigning identical text still dirties the mesh, so skip it
...
public class DialogManager : MonoBehaviour
{
    ...
    {
        var dialog = Instantiate(dialogPrefab, dialogCanvas.transform);
    ...
    {
        backdrop.SetActive(_dialogStack.Count > 0);
    }
}
```
