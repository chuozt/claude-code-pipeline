...
description: "Full audit of where the project stands: which pipeline docs exist, which are missing or stale, and what stage that implies, with a gap list and next steps; can write the report to a file. Deeper and slower than /project-overview (a read-only snapshot) and meant for a project already under way. Use when typing /project-stage-detect or saying 'where are we in development', 'what stage are we in', 'full project audit', 'what is missing'."
...
if it exists, use its value (an explicit override set by hand). Otherwise,
...
| **Polish** | Explicit only (set by hand in `production/stage.txt`) |
| **Release** | Explicit only (set by hand in `production/stage.txt`) |
...
- **No difficulty pipeline yet?** → `/gd-workflow-help` to see which phase to start from
