...
effort: medium
description: "VContainer dependency-injection reference — LifetimeScope hierarchy, registration, constructor injection for plain C#, [Inject] for MonoBehaviours, entry points. Use when registering a service, wiring a scope, or saying 'inject this', 'VContainer scope', 'why is this null after injection'."
---
...
> **Kit note.** Imported from *everything-claude-unity* (MIT).
> The C# snippets illustrate the **API only**. Where they differ from this project's rules, the
> project wins: `[SerializeField] private` with **no** underscore, `GameDebug` instead of
> `Debug.Log`, pooling instead of Instantiate/Destroy, no hardcoded gameplay numbers, logic
> outside MonoBehaviour (`coding_convention.md`, `anti-patterns.md`, `architect.md` §1).
...
If the project already has `LifetimeScope`s, register into them rather than creating new
roots. The scopes, and which of them belong to an immutable framework layer, are listed in
`project_setup.md` §1 (DI row); the game code root is in `CLAUDE.md` §1.
