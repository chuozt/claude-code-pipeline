---
name: quick-design
description: "Short design spec for a SMALL change — a tuning adjustment, a minor mechanic, a balance tweak — when a full GDD would be overkill or the system's GDD already exists. Use when typing /quick-design or saying 'small design change', 'spec this tweak', 'just a quick spec'."
...
Next step: This spec is ready to be embedded in implementation work before
...
- Reference this spec from the implementation work item so the requirement is traceable
- Implement it through the normal workflow (`.claude/workflow.md`: blueprint → approval → code → verification)
