...
> **Kit note.** Imported from *nowsprinting/unity-coding-skills*
> (Unlicense). Two overrides: (1) the pure-C# **Model** layer IS tested in **EditMode** here — that
> is the whole point of `architect.md` §1 — so ignore "never use Edit Mode for runtime logic";
> (2) `AskUserQuestion` / `test-designer` agent do not exist in this kit — ask in plain chat.
> Test naming follows `verification.md` §2.
