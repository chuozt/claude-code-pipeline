...
Unity test patterns — when a story needs an EditMode or PlayMode test file,
you can scaffold it.
...
6. **Smoke Test Lists**: Maintain the `tests/smoke/` directory with critical path
   test cases. These are the 10-15 scenarios that run in the the smoke check step gate
...
Before writing any test, classify the story type per `.claude/coding_convention.md`:
